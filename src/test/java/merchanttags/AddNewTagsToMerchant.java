...
public class AddNewTagsToMerchant {
	...
	public void testAddNewTagsToMerchant() throws IOException {
		...
		Response response = given().header("content_language", "en-US").contentType("application/json").body(Files.readAllBytes(Paths.get(json_file_path))).when().post(GetURL.readToken()+path+"/"+id);		
